...
r4 = []
for i in range (n):
    v1, v2, v3, v4 = list(map(float, a[i].split()))
    if v1 > 0:
...
plt.plot(d[:len(r1)], r1, '-o', markersize=3, color = "blue", label = "MPI")
plt.plot(d[:len(r2)], r2, '-o', markersize=3, color = "green", label = "Jacobi")
plt.plot(d[:len(r3)], r3, '-o', markersize=3, color = "yellow", label = "Gauss")
plt.plot(d[:len(r4)], r4, '-o', markersize=3, color = "purple", label = "Chebyshev_speed_MPI")
plt.legend()
...
plt.plot(t1, r1, '-o', markersize=3, color = "blue", label = "MPI")

f = open("time_jacobi.txt")
a = f.readlines()
n = len(a)
t2 = [0] * n
r2 = [0] * n
...
plt.plot(t2, r2, '-o', markersize=3, color = "green", label = "Jacobi")
...
plt.plot(t4, r4, '-o', markersize=3, color = "purple", label = "Chebyshev_MPI")
